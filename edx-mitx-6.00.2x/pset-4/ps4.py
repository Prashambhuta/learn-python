...
def r_squared(y, estimated):
    ...
    """
    y_vals = np.array(y)
    estimated_vals = np.array(estimated)
    calc_mean = np.mean(y_vals)
    estimate_error = np.sum((y_vals - estimated_vals)**2)
    variability = np.sum((y_vals - calc_mean)**2)
    r_square = 1 - (estimate_error/variability)
    return r_square
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        pylab.plot(x_vals, y_vals, 'bo', label='Observed values')
        y_estimated = np.polyval(model, x_vals)
        r_square = r_squared(y_vals, y_estimated)
        pylab.plot(x_vals, y_estimated, 'r', label='Estimated values, \nR^2 = '
                                                   + str(r_square))
        pylab.xlabel('YEAR')
